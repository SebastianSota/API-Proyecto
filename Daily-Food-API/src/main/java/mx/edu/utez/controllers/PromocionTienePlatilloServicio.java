...
import mx.edu.utez.promocionTienePlatillo.Modelo.PromocionTienePlatillo;
import mx.edu.utez.promocionTienePlatillo.Modelo.PromocionTienePlatilloDao;
...
public class PromocionTienePlatilloServicio {
    ...
    public MyResponse getPromocionTienePlatilloById(@PathParam("id") int id) throws SQLException {
        ...
        if (promocionTienePlatillo.getIdPlatillo().getIdPlatillo() > 0 && promocionTienePlatillo.getIdPlatillo().getIdPlatillo() > 0) {
            response.setCode(200);
            response.setStatus("success");
    ...
    @DELETE
    @Path("/promocion.tiene.platillo/{id,id2}")
    @Produces(MediaType.APPLICATION_JSON)
