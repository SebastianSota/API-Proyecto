...
public class PromocionDao {
    ...
    public boolean updatePromocion(Promocion promocion){
        boolean created = false;
        con = null;
        try{
            con = ConnectionDB.getConnection();
            con.setAutoCommit(false);
            ps = con.prepareStatement("UPDATE promocion SET `descripcion`= ?, `fechaInicio` = ?, `fechaFin` = ?,`status` = ?,`precio` = ? WHERE idPromocion = ?;",Statement.RETURN_GENERATED_KEYS);
            ps.setString(1,promocion.getDescripcion());
            ps.setDate(2,promocion.getFechaInicio());
            ps.setDate(4,promocion.getFechaFin());
            ps.setInt(5,promocion.getStatus());
